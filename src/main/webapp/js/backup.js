...
function generarBackup() {
    ...
    $.ajax({
        ...
        if (response.resultado === "exito") {
            // Construir mensaje de éxito con información del email
            let mensajeExito = `
            ...
            // Agregar información del correo
            if (response.email_enviado) {
                mensajeExito += `
                    <div style="background: #d4edda; padding: 10px; border-radius: 5px; margin: 10px 0;">
                        <p>📧 <strong>Correo enviado exitosamente</strong></p>
                        <p style="margin: 5px 0; font-size: 14px;">${response.detalles_email}</p>
                    </div>
                `;
            } else {
                mensajeExito += `
                    <div style="background: #fff3cd; padding: 10px; border-radius: 5px; margin: 10px 0;">
                        <p>⚠️ <strong>Backup generado pero correo no enviado</strong></p>
                        <p style="margin: 5px 0; font-size: 14px;">${response.detalles_email}</p>
                        <p style="margin: 5px 0; font-size: 12px; color: #856404;">
                            <i class="fas fa-info-circle"></i> El backup se guardó localmente en el servidor.
                        </p>
                    </div>
                `;
            }
    ...
    Swal.fire({
        ...
        html: `<div style="text-align: left;">
               ...
               <ul style="text-align: left; padding-left: 20px;">
                 ...
                 <li>El sistema estará inactivo durante la restauración</li>
                 <li>Verifique que el backup sea reciente y válido</li>
               </ul>
        ...
        if (response.resultado === 'exito') {
            Swal.fire({
                ...
                html: `<div style="text-align: left;">
                       ...
                       <p><strong>Archivo:</strong> ${response.archivo}</p>
                       <p><strong>Detalles:</strong> ${response.detalles || 'Proceso completado'}</p>
                       <div style="background: #d4edda; padding: 10px; border-radius: 5px; margin: 10px 0;">
...
function listarBackups() {
    $.ajax({
        ...
        if (response.resultado === 'exito') {
            ...
            // Aquí puedes mostrar la lista en la interfaz si lo deseas
            actualizarListaBackups(response.backups);
        } else {
...
// Función para actualizar la lista de backups en la interfaz (opcional)
function actualizarListaBackups(backups) {
    // Esta función es opcional - puedes implementarla para mostrar backups disponibles
    if (backups && backups.length > 0) {
        console.log(`📊 Se encontraron ${backups.length} backups disponibles`);
    }
}

// Función para eliminar backup (opcional)
function eliminarBackup(nombreArchivo) {
    Swal.fire({
        title: '¿Eliminar Backup?',
        text: `¿Está seguro de eliminar el backup "${nombreArchivo}"?`,
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#ef476f',
        cancelButtonColor: '#6c757d',
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                if (response.resultado === 'exito') {
                    Swal.fire('Éxito', 'Backup eliminado correctamente', 'success');
                    listarBackups(); // Actualizar lista
                } else {
                    Swal.fire('Error', response.mensaje, 'error');
                }
            }).fail(function(xhr, status, error) {
                Swal.fire('Error', 'Error al eliminar backup: ' + error, 'error');
            });
        }
    });
}
